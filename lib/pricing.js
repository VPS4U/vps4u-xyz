...
// Color mapping dla linii — używane jako wizualny identyfikator (SKU = color w VPS4U brand).
// hex: gotowy do CSS, textOnColor: kontrastowy kolor tekstu na tle hex.
const LINE_COLORS = {
  gold: { hex: '#C9A348', label: 'Gold', textOnColor: '#000' },
  orange: { hex: '#E07B00', label: 'Orange', textOnColor: '#fff' },
  czarny: { hex: '#0a0a0a', label: 'Czarny', textOnColor: '#fff' },
  bialy: { hex: '#FFFFFF', label: 'Biały', textOnColor: '#000' },
  czerwony: { hex: '#C8332E', label: 'Czerwony', textOnColor: '#fff' },
  niebieski: { hex: '#1F4E8F', label: 'Niebieski', textOnColor: '#fff' },
};

/**
 * @param {string} sku
 * @returns {{hex: string, label: string, textOnColor: string}}
 */
export function getLineColor(sku) {
  return LINE_COLORS[sku] ?? { hex: '#888', label: sku, textOnColor: '#fff' };
}
